...
debian() {
  run() { repo && install '16' '2.17.*'; }

  repo() {
    local sig; sig='/etc/apt/keyrings/timescaledb.gpg'; [[ ! -d "${sig%/*}" ]] && exit 1
    local src; src='/etc/apt/sources.list.d/timescaledb.sources'; [[ ! -d "${src%/*}" ]] && exit 1
    local key; key='https://packagecloud.io/timescale/timescaledb/gpgkey'

    curl -fsSL "${key}" | gpg --dearmor -o "${sig}" \
      && curl -fsSLo "${src}" 'https://uaik.github.io/config/apt/deb.sources.tpl' \
      && sed -i \
        -e "s|<#_name_#>|TimescaleDB|g" \
        -e "s|<#_enabled_#>|yes|g" \
        -e "s|<#_types_#>|deb|g" \
        -e "s|<#_uri_#>|https://packagecloud.io/timescale/timescaledb/debian|g" \
